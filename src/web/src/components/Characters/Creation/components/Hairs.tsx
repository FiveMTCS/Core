...
const CharacterHairs: React.FC<IArgs> = ({
	...
	const renderHairs = (currentValue: number, setter: (val: number) => void) => {
		const list: number[] = [];
		...
		return (
			<>
				{list.map((index) => (
					<HairsBox key={`hairs-${index}`} onClick={() => setter(index)}>
						<HairsImg
							src={
				...
				<ColorsWrapper>
					{hairsColors.map((color) => (
						<ColorTear
							...
							color={color.color}
							onClick={() => setSelectedPrimaryHairsColor(color.value)}
						>
				...
				<ColorsWrapper>
					{hairsColors.map((color) => (
						<ColorTear
							...
							color={color.color}
							onClick={() => setSelectedSecondaryHairsColor(color.value)}
						>
