...
const CharacterFaces: React.FC<IArgs> = ({
	...
	const renderFaces = (currentValue: number, setter: (val: number) => void) => {
		const list = [];
		...
		return (
			<>
				{list.map((index) => (
					<FaceBox key={`face-${index}`} onClick={() => setter(index)}>
						<FaceImg
							src={require(`resources/img/head/Face-${index}.jpg`).default}
						/>
						{currentValue === index && (
							<SelectionWrapper>
