...
const CharacterClothes: React.FC<IArgs> = ({
	...
	const renderTops = (currentValue: number, setter: (val: number) => void) => {
		const list: number[] = [];
		...
		return (
			<>
				{list.map((index) => (
					<ClothesBox key={`Tops-${index}`} onClick={() => setter(index)}>
						<TopsImg
							src={
	...
	const renderBottoms = (
		...
		return (
			<>
				{list.map((index) => (
					<ClothesBox key={`Bottoms-${index}`} onClick={() => setter(index)}>
						<TopsImg
							src={
	...
	return (
		...
		>
			...
			>
				...
				<ClothesWrapper>{renderTops(top, setSelectedTop)}</ClothesWrapper>
			</Accordion>
			<Accordion
