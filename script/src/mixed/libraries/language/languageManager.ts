...
class TcsLanguageManager {
		...
		this.lang =
			JSON.parse(
				LoadResourceFile(
					...
					`assets/lang/${TCS_CONFIG.lang}.json`
				)
			) || {};
	}
	...
	loadModuleLang(
		...
		const dict = JSON.parse(
			LoadResourceFile(resourceName, `assets/lang/${language}.json`)
		);
		...
		if (!dict) {
			if (language !== 'en-EN') {
				...
				this.loadModuleLang(resourceName);
			} else {
				TCS.warning(
					`${language} language file doesn't exist for module ${resourceName}.`
				);
			}
			return;
		}
