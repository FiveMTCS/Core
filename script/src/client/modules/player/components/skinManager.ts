...
const getPedMushot = async (ped: number) => {
  if (!DoesEntityExist(ped)) return

  const id = +new Date()
  const handle = RegisterPedheadshotTransparent(ped)
  let timer = 2000
  while ((!IsPedheadshotReady(handle) || !IsPedheadshotValid(handle)) && timer > 0) {
    await Delay(100)
    timer--
  }
...
const applySkin = async (character: TcsCharacterClient) => {
  const model = character.getInformations().sex === 1 ? 'mp_m_freemode_01' : 'mp_f_freemode_01'
...
on(
  ...
  ({ mugshot, id }: { mugshot: string; id: number }, cb: (json: string) => void) => {
